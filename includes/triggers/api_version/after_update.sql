CREATE TRIGGER trigger_after_update_gokabam_api_api_versions
  ...
  BEGIN
    ...
    if NEW.is_deleted = 0 THEN
      ...
      VALUES (NEW.object_id,OLD.last_page_load_id,'edit',@has_tags_changed,@has_words_changed,@has_family_changed,@has_headers_changed,@has_journals_changed);
    ELSE
      INSERT INTO gokabam_api_change_log(target_object_id,page_load_id,edit_action,is_tags,is_words,is_families,is_headers,is_journals)
      VALUES (NEW.object_id,OLD.last_page_load_id,'delete',@has_tags_changed,@has_words_changed,@has_family_changed,@has_headers_changed,@has_journals_changed);
    END IF;
    ...
    IF ((NEW.is_deleted = 1) AND (OLD.is_deleted = 0)) OR ((NEW.is_deleted = 0) AND (OLD.is_deleted = 1)) THEN
      ...
      UPDATE gokabam_api_family s SET s.is_deleted = NEW.is_deleted WHERE s.api_version_id = NEW.id;
      UPDATE gokabam_api_use_cases s SET s.is_deleted = NEW.is_deleted WHERE s.belongs_to_api_version_id = NEW.id;
      UPDATE gokabam_api_output_headers s SET s.is_deleted = NEW.is_deleted WHERE s.api_version_id = NEW.id;
