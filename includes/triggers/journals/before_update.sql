CREATE TRIGGER trigger_before_update_gokabam_api_journals
  ...
  BEGIN
    ...
    where id = NEW.target_object_id and (da_table_name = 'gokabam_api_journals' OR da_table_name = 'gokabam_api_tags');

    #dont allow Journals on Journals
    IF NEW.target_object_id AND (maybe_journal_or_tag_object IS NOT NULL)
    THEN
      SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Cannot Have a Journal On Another Tag or Journal object. Nested Journals not allowed ';
    END IF;
