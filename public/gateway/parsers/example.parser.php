...
class ParseExample {
	...
	protected static  $reference_table = 'gokabam_api_data_group_examples';
	...
	protected static function convert($manager, $node,$parent) {
		...
		$db_thing = null;
		if (is_array($node)) {
			$db_thing = new GKA_DataExample();
		} else {
			if (is_string($node)) {
				/**
				 * @var $db_thing GKA_DataExample
				 */
	 ...
	 * @param ParserManager $manager
	 * @param GKA_DataExample $db_thing
	 *
	 * @return GKA_DataExample
	 * @throws ApiParseException
	 * @throws SQLException
	...
	protected static function manage($manager, $db_thing) {
		...
		if (empty($db_thing->kid)) {
			...
			$new_id = $manager->mydb->execSQL(
				"INSERT INTO gokabam_api_data_group_examples(
						group_id,
						json_example,
						last_page_load_id,
				...
				MYDB::LAST_ID,
				'@sey@ParseExample::manage->insert'
			);
			...
			$manager->mydb->execSQL(
				...
				MYDB::ROWS_AFFECTED,
				'@sey@ParseExample::manage->update'
			);
		}
