...
class ParseInput {
	...
	protected static  $keys_to_check = ['kid','parent','origin','properties','delete'];
	...
	protected static function convert($manager, $node,$parent) {
		...
		$db_thing = null;
		if (is_array($node)) {
			$db_thing = new GKA_Input();
		} else {
			if (is_string($node)) {
				/**
				 * @var $db_thing GKA_Input
				 */
	...
	protected static function manage($manager, $db_thing) {
		...
		if (empty($db_thing->kid)) {
			...
			$new_id = $manager->mydb->execSQL(
				"INSERT INTO gokabam_api_inputs(
						api_id,
						origin_enum,
						regex_string,
						last_page_load_id,
