...
class ParseConnectionPart {
	...
	static function parse($manager, $input, $parent) {
		...
		/**
		 * @var GKA_Use_Part_Connection[] $ret
		 */
		$ret = [];
	...
	protected static function convert($manager, $node,$parent) {
		...
		$db_thing = null;
		if (is_array($node)) {
			$db_thing = new GKA_Use_Part_Connection();
		} else {
			if (is_string($node)) {
				/**
				 * @var $db_thing GKA_Use_Part_Connection
				 */
		...
		switch ($db_thing->parent->table) {
			...
			default:{
				throw new ApiParseException("parent of a connection part must be use case part");
			}
		}
	 ...
	 * @param ParserManager $manager
	 * @param GKA_Use_Part_Connection $db_thing
	 *
	 * @return GKA_Use_Part_Connection
	 * @throws ApiParseException
	 * @throws SQLException
	...
	protected static function manage($manager, $db_thing) {
		...
		if (empty($db_thing->kid)) {
			...
			$new_id = $manager->mydb->execSQL(
				"INSERT INTO gokabam_api_use_case_part_connections(
						use_case_id,
						last_page_load_id,
				...
				MYDB::LAST_ID,
				'@sey@ParseConnectionPart::manage->insert(apiversion)'
			);
			...
			$manager->mydb->execSQL(
				"UPDATE gokabam_api_use_case_part_connections SET 
								is_deleted = ?,
								use_case_id = ?,
								last_page_load_id = ?,
								initial_page_load_id = ?,
				...
				[
					...
					$db_thing->rank
						...
						MYDB::ROWS_AFFECTED,
						'@sey@ParseConnectionPart::manage->update(apiversion)'
					);
