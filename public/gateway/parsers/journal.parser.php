...
class ParseJournal {
	...
	protected static  $reference_table = 'gokabam_api_journals';
	...
	static function parse($manager, $input, $parent) {
		...
		foreach ($input as $node) {
			...
			if (empty($beer)) {continue;} //if convert decided nothing to do
	...
	protected static function manage($manager, $db_thing) {
		...
		if (empty($db_thing->kid)) {
			...
			$new_id = $manager->mydb->execSQL(
				"INSERT INTO gokabam_api_journals(
						entry,
						target_object_id,
						last_page_load_id,
					...
					MYDB::LAST_ID,
					'@sey@ParseJournal::manage->insert'
				);
			...
			$manager->mydb->execSQL(
				"UPDATE gokabam_api_journals 
					  SET entry = ?,target_object_id=?,is_deleted = ?,
					   last_page_load_id = ? WHERE id = ? ",
					[
						'siiii',
						$db_thing->text,
						$db_thing->parent->object_id,
					...
					MYDB::ROWS_AFFECTED,
				'@sey@ParseJournal::manage->update'
				);
		}
